...
predict that it will have an relatively large distance from other
...
\subsection{Outliers for for multiple related taxa}
...
<<fig=TRUE>>=
with(seqdat[Efaecium,], {
  selected <- clstutils::maxDists(dmat, idx=which(isType), N=10, exclude=outlier, include.center=TRUE)
